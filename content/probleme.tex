...
\autoref{fig:hierarchy_reduction} ist die Hierarchie der Reduktion für die ausgewählten Problemen aufgezeigt.
...
Färbung mit k Farben korrekt durchgeführt werden kann. Dieses Problem ist für $k = 2$ in \glslink{polynomialzeit}{polynomieller} Zeit lösbar, für $k \ge 2$ jedoch nicht mehr. Es gibt Spezialfälle, bei welchem auch ein 
Problem mit $k \ge 2$ in \glslink{polynomialzeit}{polynomieller} Zeit lösbar ist (siehe \cite{garey1979computers}).
...
\FloatBarrier 
	...
	\begin{itemize}
		...
		\item Färben von Landkarten: Die Länder sind die Knoten, die Kanten die Verbindung zu den Nachbarländern und die errechnete Farbe ist die Einfärbung auf der Landkarte. 
...
\end{figure}
	...
	Kanten sind Strassen. Die minimale Länge kann mit Hilfe des \glslink{eulerkreis}{Eulerkreises} relativ einfach berechnet werden. Falls der Graph die Kriterien des 
	\glslink{eulerkreis}{Eulerkreises} nicht erfüllt, werden alle Knoten mit ungeraden Anzahl von Kanten mit einem anderen solchen Knoten über die kürzeste Route verbunden. 
...
Die grün eingezeichneten Verbindungen sind Hilfslinien, um den Graph in einen \gls{eulerkreis} zu verwandeln. Eine mögliche Lösung mit der minimal Länge von 1000 ist die 
...
\subsection{Sequenzierung und Planung}\label{sequencing_scheduling}
	...
	Zusätzlich gibt es dann noch beliebige weitere Kriterien, dass zum Beispiel das eine Klasse nie mehr als 9 Lektionen pro Tag haben soll oder ein Lehrer nie mehr als 
...
sind in diesem Beispiel noch sehr überschaubar, doch es gibt bereits enorm viele verschiedenen Möglichkeiten und Ausprägungen.
...
\subsection{Mathematisches Programmieren}\label{mathematical_programming}
	...
	Beim Rucksack Problem geht es um einen Container, symbolisch der Rucksack, mit einer Gewichtsschranke. Des weiteren gibt es Objekte, welche in den Container gepackt werden 
...
\end{table}
	...
	\begin{itemize}
		...
		\item Kreditvergabe: Optimale Ausnutzung des Kreditbudgets mit der Vergabe von Krediten an Kunden, welche eine Kredit über eine gewisse Höhe haben möchte und eine 
...
Grundpfeiler der NP-Vollständigkeit, durch denn für zahlreiche Probleme die NP-Vollständigkeit bewiesen werden konnte.

	\subsubsection{Erfüllbarkeitsproblems der \gls{aussagenlogik} (SAT)}\label{sat}
	Das Erfüllbarkeitsproblem der Aussagenlogik ist ein NP-vollständiges Problem. Dies wurde durch Stephen A. Cook in den 1970er Jahren bewiesen \cite{cook_complexity}.
