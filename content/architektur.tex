...
auch dort gewiese Ähnlichkeiten. In \autoref{fig:architektur} wird der Aufbau des System gezeigt. Die \glossarmark{Domänensprache} des Nutzers wird mit Hilfe der Controller, Entities und 
den Translators auf die \glossarmark{Domänensprache} der Algorithmen abgebildet.
...
\FloatBarrier
\subsection{REST API}
Für die Schnittstelle wird ein \glossarmark{REST} \glossarmark{API} erstellt, welches die nötigen Funktionen bietet. Das \glossarmark{API} funktioniert nach dem De-facto-Standard 
(siehe \cite{wiki_restful}). Falls Fehler auftretten werden die \glossarmark{HTTP-Statuscode} verwendet, um diese an den Aufrufer weiterzugeben.
...
Die Vorteile einer \glossarmark{relationalen Datenbanksystems (RDBMS)} sind die Transaktionen und das dazugehörige \glossarmark{Atomicity, Consistency, Isolation, Durability (ACID)} Prinzip 
...
Die Abfrage der Strasse bei einer relationaler Datenbank mittels \glossarmark{Structured Query Language (SQL)} ist in \autoref{lst:select_street_rdbms} dargestellt.
...
\glossarmark{Objektrelationale Datenbanksystem (ORDBMS)} wurden entwickelt um das relationale System mit den Funktionen des objektorientierten Stiles zu erweitern.
...
Das Ziel einer \glossarmark{objektorientierten Datenbanksystems (OODBMS)} ist eine bessere und nähere Zusammenarbeit mit objektorientierten Sprachen.
Die Informationen über \glossarmark{OODBMS} sind aus \cite{limited2010introduction} entnommen. Der grosse Unterschied zu anderen Datenbanksystemen
ist die Persistierung, bei \glossarmark{OODBMS} wird bereits beim Erstellen eines neuen persistenten Objekts im Code ein Pointer auf das Objekt in der Datenbank zurückgegeben. 
...
Release migriert werden sollen. \autoref{lst:table_definition_oodbms} zeigt wie das Objekt 'Person' in \glossarmark{Object Definition Language (ODL)} definiert wird.
...
Die objektorientierten Datenbanksystemen bietet die aus den \glossarmark{RDBMS} bekannten Abfragefunktionen (Avg, Max, Min, Distinct). 
...
erinnert an \glossarmark{Binary Large Object (BLOB)} in relationalen Datenbanken, jedoch können hier Indexes auf bestimmte Attribute gesetzt und innerhalb der Elemente effizient gesucht 
werden.
